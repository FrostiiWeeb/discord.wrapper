...
class Author():
	def __init__(self, data : dict, data_member : dict = None) -> None:
		if not data_member:
			...
			self.channel = TextChannel(data["channel_id"],None,None,None,None,None,None,None,None,None,None,None)
			...
			if data["embeds"] == []:
				...
				for embed in data["embeds"]:
					self.embeds.append(embed)
			self.channel = TextChannel(data["channel_id"],data["guild_id"],None,None,None,None,None,None,None,None,None,None)
			self.name = author["username"]
			self.id = int(author["id"])
			self.discriminator = author["discriminator"]
			self.avatar = author["avatar"]